...
static bool load (char *filename, char *cmd, void (**eip) (void), void **esp);
...
process_execute (const char *filename)
{
  ...
  char *fname = NULL;
  char *saveptr;
  uint16_t arg_size, counter;
  int mult_spaces, argc;
  bool space_mode;
  ...
  strlcpy (fn_copy, filename, PGSIZE);	// copy filename to page fn_copy
  strlcpy (fname, filename, PGSIZE);
  fname = strtok_r (fname, " ", &saveptr);

  // counting arguments and calculate size on stack
  arg_size = strlen(fn_copy)+1;
  counter = 0;
  argc = 1;
  space_mode = true;
  mult_spaces = 0;
  ...
  arg_size = arg_size - mult_spaces;

  // Avoiding page faults
  // argument_size + word_align + arg_pointer + argv0 + argc + return + thread_size
  if ((arg_size + 4 + (argc+1)*4 + 4 + 4 + 4 + sizeof(struct thread)) > PGSIZE)
	goto done;

  aux_data->cmd = fn_copy;
  aux_data->filename = fname;
  aux_data->parent_thread = thread_current ();
  aux_data->new_process = NULL;
  sema_init (&aux_data->startup_sem, 0);

  /* Create a new thread to execute FILE_NAME. */
  tid = thread_create (fname, PRI_DEFAULT, start_process, aux_data);
  if (tid == TID_ERROR)
    goto done;
...
start_process (void *aux)
{
  ...
  if (! load (aux_data->filename, aux_data->cmd, &if_.eip, &if_.esp)) {
    thread->process = NULL;
  } else {
...
bool
load (char *file_name, char *cmd, void (**eip) (void), void **esp)
{
  struct thread *t = thread_current ();
...
setup_stack (void **esp, char *command)
{
  ...
  // calculating argv[0] stack offset
  // every whitespace will be changed to a \0
  arg_size = strlen(command)+1;

  // counting arguments and skipping multiple spaces
  counter = 0;
  argc = 1;
  space_mode = true;
  mult_spaces = 0;
  ...
  // calculating argc_offset: argument_size + word_align + argument_pointer + argv[0]_pointer + argc 
  mem_offset = arg_size + (4-(arg_size % 4)) + (argc + 1) * 4 + 4 + 4;

  // PUSH argc
  argc_ptr = (int*) (PHYS_BASE - mem_offset);
  // printf("Limit calculation: mem_offset (%d) + size_struct (%d) = %d < PGSIZE (%d)\n",mem_offset, sizeof(struct thread), mem_offset + sizeof(struct thread), PGSIZE);
  //if ((mem_offset + sizeof(struct thread)) > PGSIZE) {
//	return false;
  //}
  *argc_ptr = argc;

  // PUSH argv[0] pointer pointer
  argv0_ptr = (char***) (PHYS_BASE - mem_offset + 4);
  *argv0_ptr = (char**) (PHYS_BASE - mem_offset + 4 + 4);
    
  // PUSH argv[0-n] pointer and argv[0-n] values
  argv_ptr = *argv0_ptr;
  token_ptr = (char*) (PHYS_BASE - arg_size);
  counter = 0;
  token = strtok_r(command, " ", &saveptr);
  while (token != NULL) {
	counter++;
	strlength = strlcpy(token_ptr, token, PGSIZE);
	*argv_ptr = token_ptr;
	token = strtok_r(NULL, " ", &saveptr);
	token_ptr = (char*) (token_ptr + strlength + 1);
	argv_ptr = (char**) (argv_ptr + 1);
  }

  *argv_ptr = NULL; // Pushing NULL-pointer for argv[argc]

  // Set stack pointer to return adress
  *esp = PHYS_BASE - mem_offset - 4;
